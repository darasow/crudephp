...
if(isset($idPersonne))
{
    $personne = personne($idPersonne)[0];
}
if(isset($_POST['modifier']))
{
    if(controlleVide($_POST['nom'], $_POST['dateEmbauche'], $_POST['dateNaissance'], $_POST["salaire"], $_POST["adresse"], $_POST['telephone'], $_POST['email']))
    {
        ...
        $resultat = miseAjour($_POST['nom'], $_POST['dateEmbauche'], $_POST['dateNaissance'], $_POST["salaire"], $_POST["adresse"], $_POST['telephone'], $_POST['email'], $_POST["genre"], $_POST['post'], $idPersonne);
        if($resultat == 1) $succes = "Modification effectuez avec succes";
    ...
    <?php endif ?>
                ...
                <div class="md:grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6 px-4 w-[80%] mx-auto md:w-full">
                    ...
                    <div class="text-center py-2 relative">
                        ...
                        <input type="text" value="<?=$personne->nom?>"  name="nom" id="nom" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Nom">
                    </div>
                    ...
                    <div class="text-center py-2 relative">
                        ...
                        <input type="date" value="<?=$personne->dateEmbauche?>"   name="dateEmbauche" id="dateEmbauche" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Date d'embauche">
                    </div>
                    <!-- *************** dateNaissance **************** -->
                    <div class="text-center py-2 relative">
                        ...
                        <input type="date" value="<?=$personne->dateNaissance?>"   name="dateNaissance" id="dateNaissance" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Date de naissance">
                    </div>
                    <!-- *************** Salaire **************** -->
                    <div class="text-center py-2 relative">
                        <label for="salaire" class=" text-sm md:text-lg <?= ($vide)? "after:content-['*'] after:text-red-500 after:text-2xl" : "" ?>">Salaire : </label>
                        <input type="number" value="<?=$personne->salaire?>"   name="salaire" id="salaire" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Salaire">
                    </div>
                    <!-- *************** Adresse **************** -->
                    <div class="text-center py-2 relative">
                        <label for="adresse" class=" text-sm md:text-lg <?= ($vide)? "after:content-['*'] after:text-red-500 after:text-2xl" : "" ?>">Adresse : </label>
                        <input type="text" value="<?=$personne->adresse?>"   name="adresse" id="adresse" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Adresse">
                    </div>
                    <!-- *************** Telephone **************** -->
                    <div class="text-center py-2 relative">
                        <label for="telephone" class=" text-sm md:text-lg <?= ($vide)? "after:content-['*'] after:text-red-500 after:text-2xl" : "" ?>">Telephone : </label>
                        <input type="text" value="<?=$personne->telephone?>"   name="telephone" id="telephone" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Telephone">
                    </div>
                    <!-- *************** image **************** -->
                    <div class="text-center py-2 relative">
                        <label for="">Image actuel</label>
                        <img src="../../Assets/image/<?=$personne->image?>" class="w-[200px] h-[200px]" alt="" srcset="">
                    </div>
                    <div class="text-center py-2 relative">
                    ...
                    <div class="text-center py-2 relative">
                        ...
                        <input type="email" value="<?=$personne->email?>"   name="email" id="email" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10" placeholder="Email">
                    </div>
                    ...
                    <div class="text-center py-2 relative">
                        <label for="genre" class=" text-sm md:text-lg <?= ($vide)? "after:content-['*'] after:text-red-500 after:text-2xl" : "" ?>">Genre : </label>
                         <select name="genre"  id="genre" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10">
                            <option <?= ($personne->genre == "Masculin")? "selected":"" ?>  value="Masculin">Masculin</option>
                            <option <?= ($personne->genre == "Feminin")? "selected":"" ?> value="Feminin">Feminin</option>
                         </select>
                    </div>
                    <!-- ************* Post ****************** -->
                    <div class="text-center py-2 relative">
                        <label for="post" class=" text-sm md:text-lg <?= ($vide)? "after:content-['*'] after:text-red-500 after:text-2xl" : "" ?>">Post : </label>
                         <select name="post" id="post" class="text-2xl rounded-lg w-full  py-1 border-black bg-neutral-200 focus:scale-105 focus:delay-200 focus:duration-100 text-black pl-4 pr-10">
                           ...
                           <option <?php if($post->id == $personne->poste):?> selected value="<?=$personne->poste?>" <?php else: ?> value="<?= $post->id ?>" <?php endif ?>><?=($post->id == $personne->poste)? $post->nom : $post->nom ?></option>
                           <?php endforeach ?>
                         </select>
                    </div>
                ...
                <div class="w-[70%] mx-auto flex items-center justify-around">
                    ...
                    <button type="submit" name="modifier" class="text-sm md:text-lg my-4 flex items-center justify-center text-white block  uppercase bg-lime-600 rounded-md p-3 text-center">Modifier</button>
